			...
			<div className={`${styles.Kv_contents} ${"Kv_contents"}`}>
				<picture>
					...
					{isLoading ? null : (
						<>
							<source
								srcSet={`${formatImg(kv[0].url)} 1x, ${formatImg(kv[0].url)} 2x`}
								alt="KV"
								type="image/webp"
								media="(min-width: 768px)"
							/>
							<source
								srcSet={`${formatImg(kv[1].url)} 1x, ${formatImg(kv[1].url)} 2x`}
								alt="KV"
								type="image/webp"
								media="(max-width: 767px)"
							/>
							<source
								srcSet={`${kv[0].url} 1x, ${kv[0].url} 2x`}
								alt="KV"
								type="image/webp"
								media="(min-width: 768px)"
							/>
							<source
								srcSet={`${kv[1].url} 1x, ${kv[1].url} 2x`}
								alt="KV"
								type="image/webp"
								media="(max-width: 767px)"
							/>
